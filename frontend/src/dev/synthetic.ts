...
export const SYNTHETIC: {id: string; title: string; checks: number[]; table: () => TablePayload}[] = [
  {id: 'S1', title: 'Mixed currencies: header without a code, cells with their own codes; null amount sorts last', checks: [1, 3, 4, 15], table: () => synthetic(
    ...
    {metadata: {version: 2, currency: {code: null, mixed: true, codes: {EUR: 2, USD: 1}, source: 'mixed'}, complete: {rows: 3, opportunities: 3}, warnings: [{code: 'quality_warning', count: 1, message: '1 opportunity has data checks.', records: [{opportunity_no: '000123'}]}], freshness: null, fingerprint: null, explicit_columns: false}})},
