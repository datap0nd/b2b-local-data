    ...
    def test_live_http_actions_supporting_and_artifacts_are_owner_scoped(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                aggregate_review = local.request(f'/api/test/runs/{run_id}/review?start=4&size=1')
                self.assertIn('supporting', aggregate_review['steps'][0]['data'])
                self.assertTrue(aggregate_review['steps'][0]['data']['supporting']['ok'])
