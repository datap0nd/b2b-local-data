...
SQL uses certificate validation by default (`DB_SSL=true`); point `DB_CA_FILE` to an internal CA certificate if needed. `DB_SSL=prefer` matches the data-governance connection: TLS without certificate checks when the server offers it, plain text otherwise; it is selected automatically when the connection comes from `PGHOST`. `DB_SSL=false` disables TLS. A failed read reports the driver's own diagnosis with the password redacted. The application uses a read-only transaction, a statement timeout, and a maximum row bound. Its credentials need SELECT access, not migration privileges.
